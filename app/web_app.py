...
from s3_service import upload_file, list_files, download_file, S3_PREFIX
...
def download():
    ...
    success = download_file(s3_key, str(local_destination))

    if not success:
        return redirect(url_for("index"))

    return send_file(local_destination, as_attachment=True)
